...
******
      ...
      real     cofa(max_levs)     !    \
      real     cofb(max_levs)     ! parametres definissant les differentes
      real(8), pointer :: ca_m(:) ! saveurs de la coordonnee hybride utilisee
      real(8), pointer :: cb_m(:) ! dans les fichiers CMC/RPN
      real(8), pointer :: ca_t(:) !    /
      real(8), pointer :: cb_t(:) !   /
...
*-----------------------------------------------------------------------
 ...
 6010 format(/'Mauvaise valeur pour -',a/a)
      ...
      if (tlbl .or. ccc_pktyp(1:2) == 'SQ') then
         ...
         grosse_date = int( dtpr,8 )* 10000 00 00 + tmpr
